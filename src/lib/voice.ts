...
import { VOICE_SYSTEM, voiceUserPrompt } from "./prompts/voice";
...
// Prompts live in ./prompts/voice.ts (edit them there).
...
export async function analyzeVoice(input: AnalyzeVoiceInput): Promise<VoiceProfile> {
  ...
  const { data } = await llm.generateJSON({
    model: input.model,
    system: VOICE_SYSTEM,
    messages: [{ role: "user", content: voiceUserPrompt(input.sample) }],
    maxTokens: 4000,
    effort: "high",
    thinking: true,
